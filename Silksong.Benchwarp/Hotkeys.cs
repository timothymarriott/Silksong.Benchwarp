    ...
    public static class Hotkeys
    {
        ...
        internal static void AddHotkey(this Dictionary<string, int> dict, string code, int id)
        {
            if (Benchwarp.GS.HotkeyOverrides.TryGetValue(code, out string altCode))
            {
                ...
                {
                    Benchwarp.log.LogError($"Invalid hotkey override {altCode} for {code}: hotkeys must consist of exactly two letters.");
                }
                else
                {
                    code = altCode;
            ...
            {
                Benchwarp.log.LogError($"Duplicate binding for hotkey '{code}'");
            }
        }
        ...
        public static void DoHotkeyAction(int actionID)
        {
            ...
            else if (_customHotkeyActions.TryGetValue(actionID, out Action a))
            {
                ...
                {
                    Benchwarp.log.LogError($"Error invoking custom hotkey action:\n{e}");
                }
                return;
            }
            else
            {
                switch (actionID)
                {
                    ...
                    case DeployBenchID:
                        ...
                        return;
                    default:
                        Benchwarp.log.LogError($"Unknown internal hotkey code: {actionID}");
                        return;
                }
            }
        }
