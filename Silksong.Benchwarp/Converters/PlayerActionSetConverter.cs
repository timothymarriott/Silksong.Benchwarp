            ...
            while (reader.TokenType == JsonToken.PropertyName)
            {
                ...
                if (ac != null)
                {
                    if (reader.Value is string val)
                    {
                        ...
                        {
                            Benchwarp.Benchwarp.log.LogWarning($"Invalid keybinding {val}");
                        }
                    }
                    else
                    {
                        Benchwarp.Benchwarp.log.LogWarning($"Expected string for keybind, got `{reader.Value}");
                    }
                }
                else
                {
                    Benchwarp.Benchwarp.log.LogWarning($"Invalid keybind name {name}");
                }
                // JsonToken.PropertyName
                reader.Read();
            }
