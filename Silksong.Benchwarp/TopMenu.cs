...
namespace Benchwarp
{
    ...
    public static class TopMenu
    {
        ...
        private static float BottomMenuButtonsY = 400f;

        private static readonly Dictionary<string, (UnityAction<string>, Vector2)> Buttons = new()
        {
            ["Deploy"] = (DeployClicked, new Vector2(-154f, BottomMenuButtonsY)),
            ["Set"] = (SetClicked, new Vector2(-54f, BottomMenuButtonsY)),
            ["Destroy"] = (s => BenchMaker.DestroyBench(), new Vector2(46f, BottomMenuButtonsY)),
        };
        ...
        public static void BuildMenu(GameObject _canvas)
        {
            ...
            if (Benchwarp.GS.EnableDeploy)
            {
                foreach (KeyValuePair<string, (UnityAction<string>, Vector2)> pair in Buttons)
                {
                    ...
                    (
                        ...
                        buttonRect,
                        GUIController.Instance.TrajanBold,
                        Localization.Localize(pair.Key),
                        fontSize: 11
                    );
                }

                CanvasPanel nearStyle = MakePanel("Near Style", new Vector2(145f, BottomMenuButtonsY + 20f));
                {
                    Vector2 position = new(5f, 25f);

                    foreach (string styleName in BenchStyle.StyleNames)
                    {
                        AddButton(nearStyle, styleName, NearStyleChanged, position, Localization.Localize(styleName));
                ...
                CanvasPanel farStyle = MakePanel("Far Style", new Vector2(245f, BottomMenuButtonsY + 20f));
                {
                    Vector2 position = new(5f, 25f);

                    foreach (string styleName in BenchStyle.StyleNames)
                    {
                        AddButton(farStyle, styleName, FarStyleChanged, position, Localization.Localize(styleName));
                ...
                CanvasPanel options = MakePanel("Options", new Vector2(345f, BottomMenuButtonsY + 20f));
            ...
            int panelX = -1;
            int currentRowHighestCount = 0;

            Dictionary<string, Vector2> panelButtonHeight = new();
            benchPanels = new List<string>();

            Dictionary<string, int> AreaBenchCounts = new Dictionary<string, int>();

            for (int j = 0; j < Bench.Benches.Count; j++)
            {
                Bench bench = Bench.Benches[j];
                
                if (AreaBenchCounts.TryGetValue(bench.areaName, out int value))
                {
                    AreaBenchCounts[bench.areaName] = value + 1;
                }
                else
                {
                    AreaBenchCounts.Add(bench.areaName, 1);
                }
            }

            AreaBenchCounts["Other"] = 0;
            ...
            for (int i = 0; i < Bench.Benches.Count; i++)
            {
                ...
                if (!panelButtonHeight.ContainsKey(bench.areaName))
                {
                    ...
                    panelX++;
                    if (AreaBenchCounts[bench.areaName] > currentRowHighestCount)
                    {
                        currentRowHighestCount = AreaBenchCounts[bench.areaName];
                    }
                    if (panelX >= 12)
                    {
                        panelX = 1;
                        panelDistance += new Vector2(0f, 60f + currentRowHighestCount * 40);
                        currentRowHighestCount = 0;
                        panelDistance.x = -55f + 100;
                    }
        ...
        public static void Update()
        {
            ...
            if (gs.EnableDeploy)
            {
                ...
                {
                    deploy.UpdateText(Localization.Localize("Deploy"));
                    onCooldown = false;
                }
